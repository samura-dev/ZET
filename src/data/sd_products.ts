...
const sd_slugify = (sd_value: string): string =>
  sd_value
    .trim()
    .toLowerCase()
    .replace(/\s+/g, "-")
...
export const sd_products: sd_Product[] = sd_rawProducts.map((sd_item) => ({
  ...sd_item,
  slug: sd_item.slug || sd_slugify(sd_item.title),
  images: sd_item.images.map(sd_normalizeImagePath)
}));

export const sd_formatPrice = (sd_price: number): string =>
  `${new Intl.NumberFormat("ru-RU").format(sd_price)} ₽`;

export const sd_getProductBySlug = (sd_slug: string): sd_Product | undefined =>
  sd_products.find((sd_product) => sd_product.slug === sd_slug);

export const sd_getProductsByCategory = (sd_category: sd_ProductCategory): sd_Product[] =>
  sd_products.filter((sd_product) => sd_product.category === sd_category);
...
export const sd_getRelatedProducts = (sd_slug: string, sd_limit = 3): sd_Product[] => {
  ...
  return sd_products
    ...
    .sort((sd_a, sd_b) => {
      const sd_aScore =
        Number(sd_a.category === sd_current.category) +
        Number(sd_a.isBest === sd_current.isBest) +
        Number(sd_a.isNew === sd_current.isNew);
      const sd_bScore =
