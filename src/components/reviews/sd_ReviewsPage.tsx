...
export const SdReviewsPage = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    const sd_ctx = gsap.context(() => {
      gsap.fromTo(
        ".sd-reviews__hero, .sd-reviews__layout > *",
        { autoAlpha: 0, y: 26 },
        {
          autoAlpha: 1,
          y: 0,
  ...
  return (
    <section className="sd-reviews" aria-label="Отзывы" ref={sd_rootRef}>
      <header className="sd-reviews__hero">
        <h1>отзывы</h1>
        <p>Реальные мнения клиентов о моделях HUSH / ZET.</p>
      </header>

      <div className="sd-reviews__layout">
        <div className="sd-reviews__list">
          {sd_reviewsWithProduct.map((sd_review) => (
            <article className="sd-reviews__item" key={sd_review.id}>
              <div className="sd-reviews__item-head">
                <strong>{sd_review.name}</strong>
                <span>{new Date(sd_review.createdAt).toLocaleDateString("ru-RU")}</span>
              </div>
              <p className="sd-reviews__item-product">товар: {sd_review.product?.title ?? sd_review.productSlug}</p>
              <p className="sd-reviews__item-rating">
                рейтинг:
                <span className="sd-reviews__rating-pill" aria-label={`Рейтинг ${sd_review.rating} из 5`}>
                  {sd_review.rating}
                </span>
                <span>/5</span>
              </p>
              <p className="sd-reviews__item-text">{sd_review.text}</p>
            </article>
          ))}
        </div>

        <form className="sd-reviews__form" onSubmit={sd_handleSubmit}>
          ...
          <label>
            товар
            <div className="sd-reviews__select" ref={sd_productMenuRef}>
              <button
                className={
                  sd_isProductMenuOpen
                    ? "sd-reviews__select-trigger sd-reviews__select-trigger--open"
                    : "sd-reviews__select-trigger"
                }
                type="button"
                onClick={() => {
              ...
              >
                ...
                <span className="sd-reviews__select-chevron" aria-hidden>
                  ▾
                </span>
              </button>
              {sd_isProductMenuOpen ? (
                <div className="sd-reviews__select-menu">
                  {sd_products.map((sd_product) => (
                    <button
                      className={
                        sd_product.slug === sd_productSlug
                          ? "sd-reviews__select-option sd-reviews__select-option--active"
                          : "sd-reviews__select-option"
                      }
                      key={sd_product.id}
                      type="button"
                      onClick={() => {
          ...
          <fieldset className="sd-reviews__rating-fieldset">
            <legend>рейтинг (0–5)</legend>
            <div className="sd-reviews__rating-grid" role="radiogroup" aria-label="Выбор рейтинга">
              {sd_ratingSteps.map((sd_step) => (
                <button
                  key={sd_step}
                  className={
                    sd_rating === sd_step
                      ? "sd-reviews__rating-button sd-reviews__rating-button--active"
                      : "sd-reviews__rating-button"
                  }
                  type="button"
                  role="radio"
