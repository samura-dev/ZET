...
const sd_expandProductsToNine = (sd_items: sd_CatalogProduct[]): sd_CatalogProduct[] =>
  Array.from({ length: 9 }, (_, sd_index) => ({ ...sd_items[sd_index % sd_items.length], id: `${sd_items[sd_index % sd_items.length].id}-${sd_index + 1}` }));

export const SdCatalogSection = (): JSX.Element => {
  const [sd_activeTab, sd_setActiveTab] = useState<sd_TabKey>("new");
  const [sd_activeSlide, sd_setActiveSlide] = useState(0);
  const sd_addItem = sd_useCartStore((sd_state) => sd_state.sd_addItem);
  const [sd_emblaRef, sd_emblaApi] = useEmblaCarousel({ align: "center", loop: true, dragFree: false, duration: 40 });

  const sd_productsByTab = useMemo<Record<sd_TabKey, sd_CatalogProduct[]>>(() => {
    const sd_toCatalogProduct = (sd_item: (typeof sd_products)[number]): sd_CatalogProduct => ({
      id: sd_item.id,
      title: sd_item.title,
    ...
    return {
      bloggers: sd_expandProductsToNine((sd_getProductsByCategory("bloggers").map(sd_toCatalogProduct)).length ? sd_getProductsByCategory("bloggers").map(sd_toCatalogProduct) : [sd_fallback]),
      new: sd_expandProductsToNine((sd_getProductsByCategory("new").map(sd_toCatalogProduct)).length ? sd_getProductsByCategory("new").map(sd_toCatalogProduct) : [sd_fallback]),
      best: sd_expandProductsToNine((sd_getProductsByCategory("best").map(sd_toCatalogProduct)).length ? sd_getProductsByCategory("best").map(sd_toCatalogProduct) : [sd_fallback])
    };
  }, []);
    ...
    <section className="sd_catalog" id="catalog" aria-label="Каталог сумок HUSH">
      <div className="sd_catalog__tabs" role="tablist" aria-label="Группы товаров">
        {sd_tabs.map((sd_tab) => {
          ...
          return <button key={sd_tab.key} type="button" role="tab" aria-selected={sd_isActive} aria-controls={`sd_catalog-panel-${sd_tab.key}`} className={["sd_catalog__tab", sd_tab.tone === "center" ? "sd_catalog__tab--center" : "sd_catalog__tab--side", sd_isActive ? "sd_catalog__tab--active" : ""].filter(Boolean).join(" ")} onClick={() => sd_setActiveTab(sd_tab.key)}>{sd_tab.label}</button>;
        })}
      </div>

      <div className="sd_catalog__slider" id={`sd_catalog-panel-${sd_activeTab}`} role="tabpanel">
        <div className="sd_catalog__viewport" ref={sd_emblaRef}>
          <div className="sd_catalog__track">
            {sd_productsList.map((sd_product, sd_index) => {
              ...
              return (
                <div className={sd_isCentered ? "sd_catalog__slide sd_catalog__slide--active" : "sd_catalog__slide"} key={sd_product.id}>
                  <article className="sd_catalog__card" onClick={() => sd_emblaApi?.scrollNext()} role="button" tabIndex={0} onKeyDown={(sd_event) => {
                    ...
                    <div className="sd_catalog__card-visual">
                      ...
                      <img className="sd_catalog__card-image" src={sd_product.imageSrc} alt={sd_product.imageAlt} width={385} height={350} loading="lazy" decoding="async" />
                    </div>
                    <div className="sd_catalog__card-footer">
                      ...
                      <div className="sd_catalog__card-actions">
                        <button className="sd_catalog__card-add-btn" type="button" aria-label="Добавить в корзину" onClick={(sd_event) => {
                          sd_event.stopPropagation();
                          const sd_originalId = sd_product.id.includes("-") ? sd_product.id.substring(0, sd_product.id.lastIndexOf("-")) : sd_product.id;
                          ...
                          <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" width="20" height="20"><path d="M12 5V19M5 12H19" stroke="currentColor" strokeWidth="2" strokeLinecap="round" /></svg>
                        </button>
                        <a className="sd_catalog__card-cta" href={sd_product.href} onClick={(sd_event) => sd_event.stopPropagation()}>К товару</a>
                      </div>
                    </div>
                  </article>
                </div>
