...
export const SdCheckoutSection = (): JSX.Element => {
  ...
  // Состояние формы
  const [sd_formData, sd_setFormData] = useState({
    sd_fullName: "",
    sd_email: "",
    sd_phone: "",
    sd_city: "",
    sd_address: "",
    sd_deliveryMethod: "courier" as "courier" | "pickup",
    sd_paymentMethod: "card" as "card" | "cash",
    sd_promoCode: ""
  });
  ...
  const sd_handleInputChange = (sd_e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = sd_e.target;
    sd_setFormData(sd_prev => ({ ...sd_prev, [name]: value }));
    if (sd_errors[name]) {
      sd_setErrors(sd_prev => {
        const sd_new = { ...sd_prev };
        delete sd_new[name];
        return sd_new;
      });
    }
  ...
  const sd_applyPromo = () => {
    const sd_code = sd_formData.sd_promoCode.toUpperCase();
    if (sd_code === "HUSH10") {
  ...
  const sd_validate = () => {
    ...
    if (!sd_formData.sd_fullName.trim()) sd_newErrors.sd_fullName = "введите имя";
    if (!sd_formData.sd_email.includes("@")) sd_newErrors.sd_email = "неверный email";
    if (!sd_formData.sd_phone.trim()) sd_newErrors.sd_phone = "введите телефон";
    if (!sd_formData.sd_address.trim()) sd_newErrors.sd_address = "введите адрес";
    
    sd_setErrors(sd_newErrors);
    return Object.keys(sd_newErrors).length === 0;
  };

  const sd_handleSubmit = (sd_e: React.FormEvent) => {
    sd_e.preventDefault();
    if (sd_validate()) {
    ...
    <section className="sd_checkout">
      ...
      <div className="sd_checkout__stripe">
        <div className="sd_checkout__stripe_track">
          {[1,2,3,4,5,6].map(i => (
            <div key={i} className="sd_checkout__stripe_item">CHECKOUT • HUSH PREMIUM</div>
          ))}
        </div>
      </div>
          ...
          <div className="sd_checkout__group">
            ...
            <div className="sd_checkout__fields">
              <div className="sd_checkout__field">
                ...
                />
                {sd_errors.sd_fullName && <p className="sd_checkout__field_error">{sd_errors.sd_fullName}</p>}
              </div>
              <div className="sd_checkout__field">
                <span>email</span>
                <input 
                ...
                />
                {sd_errors.sd_email && <p className="sd_checkout__field_error">{sd_errors.sd_email}</p>}
              </div>
              <div className="sd_checkout__field sd_checkout__field--full">
                ...
                />
                {sd_errors.sd_phone && <p className="sd_checkout__field_error">{sd_errors.sd_phone}</p>}
              </div>
            </div>
          </div>
          ...
          <div className="sd_checkout__group">
            ...
            <div className="sd_checkout__choices">
              <button 
                ...
                onClick={() => sd_setFormData(prev => ({ ...prev, sd_deliveryMethod: "courier" }))}
              >
                курьером (500 ₽)
              </button>
              <button 
                ...
                onClick={() => sd_setFormData(prev => ({ ...prev, sd_deliveryMethod: "pickup" }))}
              >
                самовывоз (бесплатно)
              </button>
            </div>
            {sd_formData.sd_deliveryMethod === "courier" && (
              <div className="sd_checkout__fields" style={{marginTop: "12px"}}>
                <div className="sd_checkout__field sd_checkout__field--full">
                  ...
                  />
                  {sd_errors.sd_address && <p className="sd_checkout__field_error">{sd_errors.sd_address}</p>}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="sd_checkout__group">
            ...
            <div className="sd_checkout__choices">
              <button 
                ...
                onClick={() => sd_setFormData(prev => ({ ...prev, sd_paymentMethod: "card" }))}
              >
                картой онлайн
              </button>
              <button 
                ...
                onClick={() => sd_setFormData(prev => ({ ...prev, sd_paymentMethod: "cash" }))}
              >
                при получении
              </button>
            </div>
        ...
        <div className="sd_checkout__order">
                ...
                <div key={sd_item.id} className="sd_checkout__order_item">
                  ...
                  <div className="sd_checkout__order_item_content">
                    <div className="sd_checkout__order_item_top">
                      <p>{sd_item.title}</p>
                      <button className="sd_checkout__item_remove" onClick={() => sd_removeItem(sd_item.id)}>×</button>
                    </div>
                    <div className="sd_checkout__order_item_bottom">
                      <div className="sd_checkout__quantity">
                        <button onClick={() => sd_updateQuantity(sd_item.id, Math.max(1, sd_item.quantity - 1))}>-</button>
                        <span>{sd_item.quantity}</span>
                        <button onClick={() => sd_updateQuantity(sd_item.id, sd_item.quantity + 1)}>+</button>
                      </div>
          ...
          <div className="sd_checkout__summary">
            <div className="sd_checkout__promo" style={{ marginBottom: '16px' }}>
              ...
              {sd_promoError && <p className="sd_checkout__promo_message sd_checkout__promo_message--error">{sd_promoError}</p>}
              {sd_appliedPromo && <p className="sd_checkout__promo_message sd_checkout__promo_message--success">скидка 10% применена</p>}
            </div>
