...
const sd_marqueeItems = Array.from({ length: 6 }, () => sd_MARQUEE_TEXT);
  ...
  return (
    <section className="sd_hero" aria-label="Главный экран HUSH">
      ...
      <div className="sd_hero__marquee" aria-hidden>
        <div className="sd_hero__marquee_track">
          {sd_marqueeItems.map((sd_item, sd_index) => (
            <span className="sd_hero__marquee_text" key={`sd_marquee_${sd_index}`}>{sd_item}</span>
          ))}
        </div>
        <div className="sd_hero__marquee_track" aria-hidden>
          {sd_marqueeItems.map((sd_item, sd_index) => (
            <span className="sd_hero__marquee_text" key={`sd_marquee_copy_${sd_index}`}>{sd_item}</span>
          ))}
        </div>
      </div>
      ...
      <div className="sd_hero__info">
        <p className="sd_hero__info_text">each hush brand bag is the perfect combination of creativity and practicality.</p>
